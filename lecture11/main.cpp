...
void display(void)
{
    ...
    // Use "ground" texture to draw the ground
    ...
    // Use "wall" texture to draw a fence
    ...
    // Rotate and draw the same fence again
    glRotatef(-90, 0, 0, 1);
    glBegin(GL_QUADS);
    glTexCoord2f(0.0f, 0.0f); glVertex3f(-6.0f, -3.0f, 0.0f);
    glTexCoord2f(0.0f, 1.0f); glVertex3f(-6.0f, -3.0f, 1.5f);
    ...
    // Swap buffers and save pixel data to file
    glutSwapBuffers();
    grab();
}
...
int main(int argc, char* argv[])
{
    ...
    printf("Ground texture ID: %u\n", texGround);
    printf("Wall texture ID: %u\n", texWall);
    texGround = load_texture("ground.bmp");
    if (texGround == 0) {
